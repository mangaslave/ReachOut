...
if (!key || !endpoint || !textAnalyticsEndpoint || !textAnalyticsKey) {
    throw new Error(
        `Missing Azure Form Recognizer credentials: ${
            !key ? "AZURE_AI_DI_KEY1" : ""
...
export default async function analyzeResume(resumeS3Url: string, clientId: number) {
    try {
            ...
            const fullname = `${clientName[0].first_name} ${clientName[0].last_name}`;
                ...
                for (const listing of Object.values(jobListings)) {
                    ...
                    const existingSummary = await db
                        ...
                        .from(summaries)
                        and(
                            eq(summaries.jobPostingId, listing.jobPostingId),
                            eq(summaries.clientId, clientId)
                        )
        ...
        if (error instanceof Error) {
            throw new Error(
                `Failed to analyze resume. Reason: ${error.message}`
            );
        }
...
function matchKeyPhrases(
    resumeKeyPhrases: string[],
    jobKeyPhrases: string[]
): { matched: string[]; missing: string[] } {
    const matched = resumeKeyPhrases.filter((phrase) =>
        jobKeyPhrases.includes(phrase.toLowerCase())
    );
    const missing = jobKeyPhrases.filter(
        (phrase) => !resumeKeyPhrases.includes(phrase.toLowerCase())
    );
