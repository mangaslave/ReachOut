...
export function DocumentOrganizationComponent() {
  ...
  const [data, setData] = useState<ResumeItem[]>([
    {
      ...
      resumeDetails: {
        ...
        experience: {
          ...
          details: "Coordinated team schedules, managed reservations, and supported front desk operations to enhance guest experience."
        },
        summary: "Business administration graduate with hands-on experience in operational support and a passion for teamwork and efficiency."
      }
    },
    {
      name: "Amber Smith",
    ...
    },
    {
      ...
      resumeDetails: {
        ...
        summary: "Experienced software developer with a strong background in technical leadership and cloud architecture."
      }
    }
  ]);
  ...
  const handleSortChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
   ...
   return (
    <div className="w-full px-6 py-4 font-sans antialiased">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {data.map((item, index) => (
              <tr key={index}>
                ...
                <td className="px-6 py-4">
                  <div className="relative inline-block">
                    <select
                      ...
                      className={`appearance-none px-4 py-1 rounded-md border text-sm font-medium pr-8 ${statusStyles[item.status].button}`}
                    >
                ...
                <td className="px-6 py-4 text-sm font-medium">{item.lastEdited}</td>
